...
def calculate_fibo_structure_indicators(
    ...
    # Используем Rolling Window для поиска максимумов и минимумов в окне
    df["is_swing_high"] = (
...
def simulate_trades_fss_roi(
    ...
    """Симуляция торговли по стратегии Fibo Structure Scalper с ROI-Driven логикой."""
    ...
    for i in range(swing_window, len(df)):  # Начинаем позже, чтобы найти Swing точки
        ...
        # --- 3. Условия Входа (Тренд + Касание Уровня + Отскок) ---

        long_entry_cond = False
        short_entry_cond = False

        if is_uptrend and is_long_wave_available:
        ...
        if is_downtrend and is_short_wave_available:
            for fibo_level in fibo_levels_dict.values():
                # Условие касания: Свеча проколола уровень (Low <= Fibo <= High)
        ...
        if not current_trade:
            ...
            tp_price_dist_usd = current_close * (
                required_price_move_for_tp_percent / 100
            )

            # Фильтр: SL должен быть больше, чем минимальная волатильность (ATR * множитель)
            # Это гарантирует, что мы не торгуем в "мертвом" рынке.
...
if __name__ == "__main__":
    ...
    # Соответствует: (2% движения цены * 10x плечо) = 20% прибыли.
    target_roi_percent = 10.0

    # Риск ROI (Процент потери от общего капитала при срабатывании SL).
    # Соответствует: (1% движения цены * 10x плечо) = 10% убытка.
    risk_roi_percent = 10.0

    # Комиссия мейкера и тейкера.
    maker_fee = 0.0002
    taker_fee = 0.0005

    # --- 2. Параметры Индикаторов ---

    # Быстрая/Медленная EMA для определения тренда
    ema_fast_len = 9
    ema_slow_len = 21

    # Окно поиска Swing High/Low. 500 баров на 1м графике = ~8 часов истории.
    swing_window = 500

    # Период ATR для фильтрации качества сигнала
    atr_exit_len = 14

    # Минимальный множитель ATR. Если требуемый SL, рассчитанный из ROI, меньше
    # (ATR * min_atr_mult), сделка игнорируется (SL слишком близко к шуму).
