...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    # --- СЕКЦИЯ ДЛЯ ТОЧНОГО ИЗМЕРЕНИЯ СКОРОСТИ NUMBA ---
    # -------------------------------------------------------------------------

    # 1. "Разогрев" Numba: Вызываем функцию один раз, чтобы Numba скомпилировала ядро
    # Это гарантирует, что мы измеряем только время выполнения, а не компиляции.
    print("--- DEBUG: Запуск 'разогрева' Numba (компиляция)...")
    _ = simulate_trades_fss_roi(
        df.copy(),  # Передаем копию, чтобы избежать непредвиденных побочных эффектов (хотя функция должна быть чистой)
        initial_capital,
        leverage,
        maker_fee,
        taker_fee,
    ...
    # 2. Фактическое измерение времени (используем perf_counter для точности)
    ...
    print(
        f"\n[TIME] Время выполнения симуляции (Numba, без компиляции): {end_time - start_time:.4f} секунд."
    )
