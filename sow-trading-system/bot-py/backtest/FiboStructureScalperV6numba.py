...
if not os.path.exists(FILE_PATH):
    ...
    # Для целей демонстрации здесь не вызываем exit(), чтобы код мог быть запущен в окружении без файла
    # exit()
...
class StrategyConfig:
    """Хранит все параметры для конкретной стратегии, включая модульные индикаторы."""
    ...
    def __init__(self, **kwargs):
        ...
        # 2. Модульная Конфигурация Индикаторов
        # Теперь параметры индикаторов задаются здесь,
        # что позволяет легко добавлять/удалять модули.
        self.indicator_set = kwargs.get(
            "indicator_set",
            {
                "EMA_TREND": {"fast_len": 9, "slow_len": 21},
                "ATR_EXIT": {"atr_len": 14, "min_multiplier": 1.5},
                "SWING_STRUCT": {"window": 500},
                "HTF_FILTER": {
                    "period": "15min",
                    "ema_fast_len": 9,
                    "ema_slow_len": 21,
                },
                # Параметры, не относящиеся напрямую к колонкам DF, но влияющие на логику
                "FIBO": {"levels": np.array([0.5, 0.618], dtype=np.float64)},
            },
        )

        # Извлечение параметров из indicator_set для совместимости с ядром Numba
        self.ema_fast_len = self.indicator_set["EMA_TREND"]["fast_len"]
        self.ema_slow_len = self.indicator_set["EMA_TREND"]["slow_len"]
        self.swing_window = self.indicator_set["SWING_STRUCT"]["window"]
        self.atr_exit_len = self.indicator_set["ATR_EXIT"]["atr_len"]
        self.atr_min_multiplier = self.indicator_set["ATR_EXIT"]["min_multiplier"]
        self.fibo_levels = self.indicator_set["FIBO"]["levels"]

        self.htf_period = self.indicator_set["HTF_FILTER"]["period"]
        self.htf_ema_fast_len = self.indicator_set["HTF_FILTER"]["ema_fast_len"]
        self.htf_ema_slow_len = self.indicator_set["HTF_FILTER"]["ema_slow_len"]
...
# =========================================================================
# === МОДУЛЬ 2: ПОДГОТОВКА ДАННЫХ И ИНДИКАТОРОВ (МОДУЛЬНЫЙ РАСЧЕТ) ===
# =========================================================================
...
def load_data(file_path):
    ...
    try:
        ...
        # Для окружения без файла, создаем мок-данные
        print(f"Файл {file_path} не найден. Генерация мок-данных.")
        dates = pd.to_datetime(
            pd.date_range(start="2024-01-01", periods=1000, freq="1min")
        )
        df = pd.DataFrame(
            {
                "timestamp": dates,
                "open": np.linspace(30000, 31000, 1000) + np.random.randn(1000) * 10,
                "high": np.linspace(30000, 31000, 1000)
                + np.random.randn(1000) * 10
                + 5,
                "low": np.linspace(30000, 31000, 1000) + np.random.randn(1000) * 10 - 5,
                "close": np.linspace(30000, 31000, 1000) + np.random.randn(1000) * 10,
...
def calculate_strategy_indicators(df, config: StrategyConfig):
    """Расчет индикаторов на основе конфигурации StrategyConfig.indicator_set."""
    ...
    if "EMA_TREND" in indicators:
        ...
        df["ema_fast"] = talib.EMA(df["close"], timeperiod=params["fast_len"])
        df["ema_slow"] = talib.EMA(df["close"], timeperiod=params["slow_len"])

    # 2. ATR для фильтрации входов
    if "ATR_EXIT" in indicators:
        params = indicators["ATR_EXIT"]
        df["atr_val"] = talib.ATR(
            df["high"], df["low"], df["close"], timeperiod=params["atr_len"]
        )
    ...
    if "HTF_FILTER" in indicators:
        ...
        df_htf["htf_ema_fast"] = talib.EMA(
            df_htf["close"], timeperiod=params["ema_fast_len"]
        )
        df_htf["htf_ema_slow"] = talib.EMA(
            df_htf["close"], timeperiod=params["ema_slow_len"]
        )
    ...
    for i in range(max(swing_window, start_index), data_len):
        ...
        if current_trade is None and (long_entry_cond or short_entry_cond):
            ...
            if sl_price_dist_usd > 0.0:
                ...
                # Максимальный размер позиции по марже
...
def run_backtest(df, config: StrategyConfig):
    ...
    # ПРИМЕЧАНИЕ: Здесь мы явно извлекаем массивы, которые требуются Numba ядру.
    # Если в config.indicator_set нет, например, HTF_FILTER, то эта колонка
    # не будет существовать в DF, и код упадет. Для следующего шага
    # (повышение модульности в Numba) потребуется проверка существования колонки.
    # Пока предполагаем, что для этой стратегии все необходимые колонки есть.
    try:
        ...
        print(f"\n[Критическая ошибка] Не найден обязательный индикатор: {e}. ")
        print(
            "Проверьте, что все модули (EMA_TREND, ATR_EXIT, SWING_STRUCT, HTF_FILTER) включены в StrategyConfig.indicator_set."
        )
...
def plot_results(trades_df, equity_curve, drawdown_for_plot, initial_capital):
...
if __name__ == "__main__":
    ...
    # 6. ГРАФИКИ
    # plot_results(trades_df, equity_curve, drawdown_for_plot, strategy_config.initial_capital)
    print(
