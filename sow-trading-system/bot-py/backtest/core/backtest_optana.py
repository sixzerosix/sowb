...
# def load_data(file_path: str = "mock_data.csv") -> pd.DataFrame:
#     """
...
#     )
#     periods = 1000  # 1000 1-минутных свечей (~1 день)
#     np.random.seed(42)
    ...
    for i in range(1, len(df)):
        ...
        if open_position is not None:
            ...
            if side == "LONG":
                ...
                # Exit by Take Profit (High >= TP)
                    ...
                    exit_reason = "TP"
                    exit_price = tp_price
                # Exit by reverse trend (RSI too high) - используем уровень выхода для RSI, если он задан
                elif current_row["rsi_val"] >= config.indicator_set.get(
                    "RSI_ENTRY_EXIT", {}
                ).get("exit_high", 70):
            ...
            if exit_reason is not None:
                pnl = (
                    (exit_price - entry_price) / entry_price * current_equity
                    if side == "LONG"
                    else 0
                )
        ...
        if open_position is None:
            ...
            if long_entry_condition:
                ...
                # Для простоты: TP - это фиксированное ATR расстояние
