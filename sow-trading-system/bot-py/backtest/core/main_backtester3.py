...
CONFIG_MAP = {
    "HFT_SCALPER": STRATEGY_CONFIG_BREAKOUT_SCALPER,
    "BREAKOUT_SCALPER": STRATEGY_CONFIG_BREAKOUT_SCALPER,
    "MR_SCALPER": STRATEGY_CONFIG_BREAKOUT_SCALPER,
}
# --- КОНЕЦ ЗАГЛУШКИ ---
...
def run_backtest(
    ...
    # Хранение текущей открытой позиции
    open_position_data = {}
    ...
    for i in range(len(df_processed)):
        ...
        long_condition = (
            fast_ema[i] > slow_ema[i] and fast_ema[i - 1] <= slow_ema[i - 1]
        )

        # --- ЛОГИКА ОТКРЫТИЯ СДЕЛКИ ---
        # Проверяем условия входа и что позиция еще не открыта
        if (
            long_condition
            and i > 1
            and np.random.rand() < 0.2
            and not open_position_data  # Проверка, что позиции нет
        ):
            ...
            # Сохраняем данные об открытой позиции
            open_position_data = {
                "side": "LONG",
                ...
                "signal_reason": "EMA_CROSS_UP",  # НОВОЕ: Причина сигнала
            }
        ...
        if open_position_data:
            ...
            if np.random.rand() < 0.3:
                current_equity += pnl
                next_close = close_prices[i]  # Цена закрытия - текущая цена

                # --- ЗАПИСЬ ЗАКРЫТОЙ СДЕЛКИ (ОБНОВЛЕНО) ---
                trades.append(
                    {
                        "entry_time": open_position_data["entry_time"],
                        "side": open_position_data["side"],
                        "entry_price": open_position_data["entry_price"],
                        "take_profit": open_position_data["take_profit"],  # НОВОЕ ПОЛЕ
                        "stop_loss": open_position_data["stop_loss"],  # НОВОЕ ПОЛЕ
                        "signal_reason": open_position_data[
                            "signal_reason"
                        ],  # НОВОЕ ПОЛЕ
                        "exit_time": df_processed["timestamp"].iloc[i],
    ...
    if open_position_data:
        ...
        # --- ЗАПИСЬ НЕЗАКРЫТОЙ СДЕЛКИ (ОБНОВЛЕНО) ---
        trades.append(
            {
                "entry_time": open_position_data["entry_time"],
                "side": open_position_data["side"],
                ...
                "take_profit": open_position_data["take_profit"],  # НОВОЕ ПОЛЕ
                "stop_loss": open_position_data["stop_loss"],  # НОВОЕ ПОЛЕ
                "signal_reason": open_position_data["signal_reason"],  # НОВОЕ ПОЛЕ
    ...
    if trades_df.empty:
        ...
        trades_df = pd.DataFrame(
            columns=[
                ...
                "side",
                "entry_price",
                "take_profit",  # Добавлено
                "stop_loss",  # Добавлено
                "signal_reason",  # Добавлено
                "exit_time",
                "exit_price",
                "pnl",
                "close_reason",
...
# =========================================================================
# === ФУНКЦИИ ОПТИМИЗАЦИИ И ОРКЕСТРАЦИИ (БЕЗ ИЗМЕНЕНИЙ) ===
# =========================================================================
...
if __name__ == "__main__":
    ...
    persistence_config = PersistenceConfig(
        ...
        save_to_csv=False,  # Включаем сохранение в CSV для демонстрации
        save_to_txt=True,
