...
def simulate_trades_pine_v3(
    ...
    for i in range(1, len(df)):
        ...
        # -------------------------------------------------------------------------
        # УСЛОВИЯ СМЯГЧЕНЫ: Проверки RSI и Delta OBV закомментированы!
        # Чтобы сделать стратегию более строгой, удалите `##` внизу.
        # -------------------------------------------------------------------------

        # Long Entry: EMA Cross Up AND close > VWAP
        long_entry_cond = (
            ema_cross_up
            and (current_close > vwap_val)
            ## and (rsi_fast > rsi_mid_level) # Дополнительное условие RSI
            ## and (delta_obv > 0)           # Дополнительное условие Delta OBV (объемы)
        )

        # Short Entry: EMA Cross Down AND close < VWAP
        short_entry_cond = (
            ema_cross_down
            and (current_close < vwap_val)
            ## and (rsi_fast < rsi_mid_level) # Дополнительное условие RSI
        ...
        if not current_trade:
            ...
            # Если комиссии выше 10% от максимального риска, не входим
            # NOTE: Эта проверка была очень строгой и, вероятно, блокировала все сделки!
            # commission_check_ok = estimated_total_commission < (max_loss_usd * 0.1)
            commission_check_ok = (
                True  # Временно отключаем строгую проверку комиссии для бэктеста
            )
