...
# Используем путь, который вы указали:
...
def simulate_trades_pine_v3(
    ...
    for i in range(1, len(df)):
        ...
        # Пересечение EMA (основной сигнал тренда)
        ...
        # -------------------------------------------------------------------------
        # УСЛОВИЯ СТРОГОСТИ ВОССТАНОВЛЕНЫ: Проверки RSI и Delta OBV ВКЛЮЧЕНЫ
        # -------------------------------------------------------------------------

        # Long Entry: EMA Cross Up AND close > VWAP AND RSI/OBV CONFIRMATION
        long_entry_cond = (
            ema_cross_up
            and (current_close > vwap_val)
            and (rsi_fast > rsi_mid_level)  # Дополнительное условие RSI (импульс)
            and (delta_obv > 0)  # Дополнительное условие Delta OBV (объемы)
        )

        # Short Entry: EMA Cross Down AND close < VWAP AND RSI/OBV CONFIRMATION
        short_entry_cond = (
            ema_cross_down
            and (current_close < vwap_val)
            and (rsi_fast < rsi_mid_level)  # Дополнительное условие RSI (импульс)
        ...
        if not current_trade:
            ...
            # NOTE: Комиссия была временно отключена, теперь возвращаем ее, но смягчаем условие.
            # Если комиссии выше 50% от максимального риска, не входим
            # commission_check_ok = estimated_total_commission < (max_loss_usd * 0.5)
            commission_check_ok = True  # Это было временное отключение
...
if __name__ == "__main__":
    ...
    # Начальный размер капитала.
    initial_capital = 100.0

    # Плечо (Leverage).
    # Увеличение (например, с 10.0 до 20.0): Увеличивает потенциальную прибыль/убыток.
    # Смягчает: Требует меньше маржи для сделки, что позволяет открыть больший объем.
    # Потенциал: Выше ROI, но выше риск ликвидации.
    leverage = 10.0

    # Комиссия мейкера (Maker Fee) и тейкера (Taker Fee).
    # Уменьшение: Смягчает, т.к. сделки становятся дешевле.
    # Потенциал: Увеличивает ROI, особенно для скальпинга.
    maker_fee = 0.0002
    taker_fee = 0.0005

    # Риск на сделку в процентах от общего капитала.
    # Увеличение (например, с 2% до 5%): Смягчает, т.к. позволяет открывать больший объем позиции.
    # Потенциал: Более быстрая кривая эквити (в обе стороны).
    risk_per_trade_percent = 2.0

    # --- 2. Параметры Индикаторов (СМЯГЧЕНИЕ УСЛОВИЙ ВХОДА) ---

    # Период быстрой EMA.
    # Уменьшение (например, с 9 до 7): Смягчает, дает более быстрые и ранние сигналы.
    # Потенциал: Больше сделок, но больше шума (ложных сигналов).
    ema_fast_len = 9

    # Период медленной EMA.
    # Уменьшение (например, с 21 до 14): Смягчает, делает кроссоверы EMA более частыми.
    # Потенциал: Больше сделок, но меньше подтверждения тренда.
    ema_slow_len = 21

    # Период RSI.
    # Уменьшение (например, с 5 до 3): Смягчает, делает RSI более чувствительным.
    # Потенциал: Более ранние сигналы, но выше риск ложного разворота.
    rsi_fast_len = 5

    # Уровень RSI для подтверждения тренда (50 - нейтральный).
    # Изменение (например, с 50 до 45 для LONG): Смягчает, позволяет входить при более слабом импульсе.
    # Потенциал: Больше сделок, но ниже качество.
    rsi_mid_level = 50

    # VWAP Proxy: Используем EMA 50 как длинную, стабильную среднюю
    # Уменьшение (например, с 50 до 30): Смягчает, делает VWAP ближе к цене.
    # Потенциал: Больше кроссоверов, больше сделок.
    vwap_len = 50

    # --- 3. Параметры Выхода (ATR Stops) ---

    # Период ATR для расчета Стоп-Лосса и Тейк-Профита.
    # Уменьшение (например, с 14 до 7): Смягчает, стопы и тейки становятся ближе к цене (более "узкие").
    # Потенциал: Меньше убытки, но чаще выбивает по SL.
    atr_exit_len = 14

    # Множитель для Стоп-Лосса (SL = ATR * X).
    # Увеличение (например, с 1.5 до 2.5): Смягчает, дает больше пространства цене (шире SL).
    # Потенциал: Реже выбивает по шуму, но выше средний убыток.
    stop_loss_atr_mult = 1.5

    # Множитель для Тейк-Профита (TP = ATR * Y).
    # Уменьшение (например, с 3.0 до 1.5): Смягчает, быстрее фиксирует прибыль (меньше TP).
    # Потенциал: Выше % прибыльных сделок, но ниже средняя прибыль.
    # Установите 0.0, если не хотите TP (выход только по SL или Reverse Signal).
    ...
    result = analyze_results(trades_df, initial_capital)
    if result:
        drawdown_data, equity_curve_data = result
        # --- Построение графиков ---
